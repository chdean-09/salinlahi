...
- **18** `Char_*.asset` files exist on disk, but only **17** are taught: `CampaignConfig_RevisedV1.symbols` holds 17 and excludes `Char_RA`, which has no spoken value and no `firstIntroductionLevelId`. The 18th file is retained for its recognition templates and art but is referenced by **nothing**: SALIN-212 removed it from `CharacterRegistry_Default.asset` on 2026-09-01, bringing that registry to 17 as well. Content-complete is measured against the **17** in the campaign catalog; see REQ-42 in doc 10.
