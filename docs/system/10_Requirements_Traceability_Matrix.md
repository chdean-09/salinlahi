...
| REQ-12 | Recognition shall cover 17 Baybayin consonant characters | Salinlahi.md §1.5.1; §3.3.3 | P0 | `TemplateLoader.cs` loads from `Resources/Templates/`; template .txt files status unverified | RC-07 | ⚠ Partial | 🟠 P1 |
...
| REQ-14 | Strokes with fewer than 8 points shall be rejected as taps | RecognitionConfigSO | P1 | `RecognitionConfigSO.minimumPointCount = 8`; `StrokeCapture.cs` | RC-06 | ✅ Implemented | None |
...
| REQ-27 | Wave spawning shall respect waveStartDelay and spawnInterval from WaveConfigSO | TDD §3.2 | P1 | `WaveSpawner.cs` reads `WaveConfigSO` | WV-02, WV-03 | ✅ Implemented | None |
