...
| REQ-12 | Recognition shall cover the Baybayin character set (**amended 2026-08-31: 18**, was "17 consonant") | Salinlahi.md §1.5.1; §3.3.3 | P0 | `TemplateLoader.cs` loads `Resources/Templates/` — **121 template files covering all 18 characters** (~6–7 variants each) | RC-07 | ✅ Implemented | None — requirement text corrected to 18 under the REQ-42 ruling |
...
| REQ-42 | Baybayin character set shall include **15 consonants and 3 vowels (A, E/I, O/U) totaling 18** (amended 2026-08-31; previously stated 17) | GDD §3.3 | P0 | **Shipped set is 18**: `A BA DA EI GA HA KA LA MA NA NGA OU PA RA SA TA WA YA`. `RA` is authored as its own glyph, where classic Baybayin folds it into `DA` | RC-07 | ✅ Implemented | **RESOLVED** — ruled 2026-08-31 that `RA` is its own glyph, so 18 is canonical and the code was always correct. GDD corrected in 9 places (not the 5 previously recorded). Remaining, tracked separately: Kadiliman is authored as a 1-phase / 3-draw stub versus its all-18 design (doc 04), and the GDD scaffolding table introduces only 14 of the 18 characters |
