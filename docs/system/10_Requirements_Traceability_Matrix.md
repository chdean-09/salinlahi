...
| REQ-12 | Recognition shall cover the Baybayin **glyph shapes** (**18** — recognition scope, distinct from the 17 taught identities in REQ-42) | Salinlahi.md §1.5.1; §3.3.3 | P0 | `TemplateLoader.cs` loads `Resources/Templates/` — **121 template files covering all 18 shapes** (~6–7 variants each; `RA` carries 5, `DA` 12) | RC-07 | ✅ Implemented | None. 18 is correct **here** — the recognizer distinguishes `RA` from `DA` by shape even though the curriculum teaches them as one identity |
