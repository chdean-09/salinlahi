...
| REQ-12 | Recognition shall cover 17 Baybayin consonant characters | Salinlahi.md §1.5.1; §3.3.3 | P0 | `TemplateLoader.cs` loads `Resources/Templates/` — **121 template files covering all 18 characters** (~6–7 variants each) | RC-07 | ✅ Implemented | None (SALIN-186: requirement text says "17 consonant"; the set is 15 consonants + 3 vowels = **18**) |
...
| REQ-42 | Baybayin character set shall include 14 consonants and 3 vowels (A, E/I, O/U) totaling 17 | GDD §3.3 | P0 | **The shipped set is 18**: 15 consonants + 3 vowels (`A BA DA EI GA HA KA LA MA NA NGA OU PA RA SA TA WA YA`). `RA` is authored as its own glyph, where classic Baybayin folds it into `DA` | RC-07 | ⚠ Partial | 🟠 P1 (SALIN-186: **the requirement itself is wrong, not the code** — GDD says 17 in 5 places; needs a content decision, see doc 13) |
