...
| D1 | Were the Ugat context challenges retired on purpose, or is the routing that skips them a defect? | **A defect. The boards are restored.** The five `ChallengeMode` values, the three `ChallengeCluePolicy` values and the authored Ugat 1-5 sequences already implement the level-type progression exactly as the reveal design specifies, including Level 5's gapped paragraph prompt. `LevelFlowController.ExecuteContextChallenge` runs `ExecuteCombatRestoration()` and then `yield break`s before the board is reached, so none of it has ever been shown. Re-enable the board for units whose mode is SentenceRestoration or ParagraphRestoration, keyed on unit mode rather than level number. WordPlacement units stay retired: slot-fill subsumes them. | Jon Wayne Cabusbusan | 2026-09-17 | WI-4 (QA plan §4.5), reconciliation plan `docs/plans/2026-09-17-challenge-reveal-reconciliation.md` |
